...
var caseless = _interopDefault(require('caseless'));
...
const parse = async res => {
  ...
  return null;
}; // standard interceptor - we won't let you de-register it


const standardResponseInterceptor = {
  response: async res => {
    const flighty = await parse(res);
    return { ...res,
      flighty
    };
  }
}; // useful interceptor that you may want to use

const errorIfNotOKInterceptor = {
  response: res => {
    if (!res.ok) {
      throw res;
    }
...
class Flighty {
  constructor(options = {}) {
    // add the methods
    METHODS.forEach(method => this[method.toLowerCase()] = (path = "/", options = {}) => call(method, this, {
      path,
      options
    }));
    let localAbortController;
    const interceptors = new Set([standardResponseInterceptor]);
  ...
  clearInterceptors() {
    this.interceptors.clear(); // this is the default response interceptor

    this.interceptors.add(standardResponseInterceptor);
  }
...
exports.errorIfNotOKInterceptor = errorIfNotOKInterceptor;
