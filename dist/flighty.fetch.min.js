...
// "abortAll" method
const h=C(e,c.abortController,c.abortTokenMap),i={...e},j={...f},k=d,l=[],m=Array.from(c.interceptors),n=a(m,Promise.resolve([d,e,{...f},g]),"request","requestError",// don't let interceptors modify the extra or retryCount data
a=>{const[b,c]=a.slice(0,2);return l.push([b,{...c}]),[b,c].concat([{...f},g])}),o=a(m.reverse(),(async()=>{// stuff from the interceptors
const[a,d]=await n;l.push([a,{...d}]);const e=await E(b,c,a,{...d,signal:h});// add in the json and text responses to extra to make life easier
// for people - they can still await them if they want
if(e.flighty={method:b,retryCount:g,// the values flighty was called with
call:{path:k,options:i,extra:j},// the values that were returned from each request interceptor - useful for debugging!
intercepted:l,// retry method
